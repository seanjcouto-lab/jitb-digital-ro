...
const App: React.FC = () => {
  ...
  const mainContent = () => {
    ...
    switch (activeRole) {
     case UserRole.SERVICE_MANAGER: return <ServiceManagerPage addRO={addRO} repairOrders={repairOrders} updateRO={updateRO} deleteRO={deleteRO} hourlyRate={config.hourlyRate} taxRate={config.taxRate} overridePin={config.overridePin} masterInventory={masterInventory} onNavigateToCalendar={() => setImpersonatedRole(UserRole.CALENDAR)} />;
