      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="flex justify-between items-center bg-slate-900/80 p-6 rounded-2xl border border-white/5">
          <div>
            ...
            <EngineIdentityLine engineYear={repairOrder.engineYear} engineMake={repairOrder.engineMake} engineModel={repairOrder.engineModel} engineHours={repairOrder.engineHours} engineHorsepower={repairOrder.engineHorsepower} className="text-[11px] font-bold text-slate-400 mt-0.5" />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    {repairOrder.parts.map((part, index) => {
                        ...
                        return (
                            <div key={part.partNumber + index} className="flex justify-between items-center p-3 bg-slate-900/50 rounded-xl border border-white/5 group">
                                <div><p className="font-bold text-sm text-slate-200">{part.description}</p><p className="text-[10px] text-slate-500 font-mono">{part.partNumber} • BIN: {part.binLocation}</p><div className="flex items-center gap-1 mt-1"><label className="text-[9px] text-slate-600 uppercase font-bold">Qty</label><input type="number" min={1} onFocus={e => e.target.select()} value={part.quantity ?? 1} onChange={e => { const updated = repairOrder.parts.map((p, i) => i === index ? { ...p, quantity: Math.max(1, Math.floor(parseInt(e.target.value) || 1)) } : p); updateRO({ ...repairOrder, parts: updated }); }} className="w-12 bg-slate-800 border border-white/10 rounded px-1 py-0.5 text-amber-400 font-bold text-[10px] text-center outline-none focus:border-amber-400" /></div></div>
          ...
          <div className="space-y-6">
            <section className="glass p-6 rounded-2xl border-white/5 h-fit">
              <SectionHeader title="Add Work & Parts" />
              <div className="space-y-6">
